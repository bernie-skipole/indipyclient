...
As devices and vectors are learnt from the received data, the IPyClient object becomes a mapping of devicename to device objects, which in turn are mappings to vectors and vector members.

The vector objects have send_newxxxxVector methods which can be used by your own code to send data to the remote instrument.
